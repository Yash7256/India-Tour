...
const BarwaniPage = () => {
  ...
  const [cityId, setCityId] = useState<string>('barwani-mp');
      ...
      switch (selectedTab) {
        ...
        case 'Local Specialties':
          try {
            ...
            console.log('Fetching local specialties for BARWANI...');
        ...
        lastUpdated: new Date().toISOString()
              ...
              const { data: caseInsensitiveResults } = await supabase
                .from('local_specialties')
                .select('id, name, description, city, state, image_url, category, created_at, updated_at, city_id')
                .or('city.ilike.%barwani%,state.ilike.%madhya pradesh%')
                .order('name', { ascending: true });
              ...
              if (caseInsensitiveResults && caseInsensitiveResults.length > 0) {
                ...
                setError('No local specialties found for BARWANI.');
                setLocalSpecialties([]);
              }
            }
          } catch (err) {
            ...
            if (!response.data || response.data.length === 0) {
              ...
              if (response.data) {
                response.data = response.data.filter((item: TransportOption) => 
                  item.city?.toLowerCase().includes('barwani') ||
                  item.name?.toLowerCase().includes('barwani') ||
                  item.description?.toLowerCase().includes('barwani')
                );
              }
            }
          } else {
            ...
            if (!response.data || response.data.length === 0) {
              ...
              if (response.data) {
                response.data = response.data.filter((item: Event) => 
                  item.city?.toLowerCase().includes('barwani ') ||
                  item.location?.toLowerCase().includes('barwani ') ||
                  item.name?.toLowerCase().includes('barwani ')
                );
              }
            }
    ...
    if (currentData.length === 0) {
      return (
        <div className="text-center py-12">
          <p className="text-gray-500">No {selectedTab.toLowerCase()} found for Barwani.</p>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
     ...
     <div className="relative h-96 overflow-hidden">
        <img
          src="https://cdn.britannica.com/62/124462-004-F8650DBF/Narmada-River-Maheshwar-Madhya-Pradesh-India.jpg"
          alt="Barwani"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="bg-white rounded-xl shadow-sm p-6">
              <h2 className="text-2xl font-bold mb-4">About BARWANI</h2>
              <p className="text-gray-600 mb-6">
              Barwani, often called the “Paris of Nimar”, is a picturesque town in Madhya Pradesh located on the banks of the Narmada River and surrounded by the Satpura hills. Known for its rich tribal culture, scenic landscapes, and historic temples, Barwani serves as a gateway to the Narmada Valley and offers a blend of natural beauty and heritage
              </p>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              <div className="bg-gray-100 rounded-lg h-48 relative overflow-hidden mb-4">
                <iframe
                  src="https://www.google.com/maps?q=Barwani,+Madhya+Pradesh,+India&output=embed"
                  width="100%"
                  height="100%"
                  style={{ border: 0 }}
