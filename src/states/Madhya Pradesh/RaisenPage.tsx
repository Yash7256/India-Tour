...
const RaisenPage = () => {
  ...
  const [cityId, setCityId] = useState<string>('raisen-mp');
      ...
      switch (selectedTab) {
        ...
        case 'Local Specialties':
          try {
            ...
            console.log('Fetching local specialties for RAISEN...');
        ...
        lastUpdated: new Date().toISOString()
              ...
              const { data: caseInsensitiveResults } = await supabase
                .from('local_specialties')
                .select('id, name, description, city, state, image_url, category, created_at, updated_at, city_id')
                .or('city.ilike.%raisen%,state.ilike.%madhya pradesh%')
                .order('name', { ascending: true });
              ...
              if (caseInsensitiveResults && caseInsensitiveResults.length > 0) {
                ...
                setError('No local specialties found .');
                setLocalSpecialties([]);
              }
            }
          } catch (err) {
            ...
            if (!response.data || response.data.length === 0) {
              ...
              if (response.data) {
                response.data = response.data.filter((item: TransportOption) => 
                  item.city?.toLowerCase().includes('raisen') ||
                  item.name?.toLowerCase().includes('raisen') ||
                  item.description?.toLowerCase().includes('raisen')
                );
              }
            }
          } else {
            ...
            if (!response.data || response.data.length === 0) {
              ...
              if (response.data) {
                response.data = response.data.filter((item: Event) => 
                  item.city?.toLowerCase().includes('raisen') ||
                  item.location?.toLowerCase().includes('raisen') ||
                  item.name?.toLowerCase().includes('raisen')
                );
              }
            }
    ...
    if (currentData.length === 0) {
      return (
        <div className="text-center py-12">
          <p className="text-gray-500">No {selectedTab.toLowerCase()} found for Raisen.</p>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
     ...
     <div className="relative h-96 overflow-hidden">
        <img
          src="https://upload.wikimedia.org/wikipedia/commons/1/10/Raisen_Fort%2C_Madhya_Pradesh.jpg"
          alt="Raisen"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="bg-white rounded-xl shadow-sm p-6">
              <h2 className="text-2xl font-bold mb-4">About RAISEN</h2>
              <p className="text-gray-600 mb-6">
              RAISEN – LAND OF FORTS & SHRINES, RICH IN HISTORY AND CULTURAL HERITAGE.
              </p>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              <div className="bg-gray-100 rounded-lg h-48 relative overflow-hidden mb-4">
                <iframe
                  src="https://www.google.com/maps?q=Raisen,+Madhya+Pradesh,+India&output=embed"
                  width="100%"
                  height="100%"
                  style={{ border: 0 }}
