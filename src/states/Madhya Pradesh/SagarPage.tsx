...
const SagarPage = () => {
  ...
  const [cityId, setCityId] = useState<string>('sagar-mp');
      ...
      switch (selectedTab) {
        ...
        case 'Local Specialties':
          try {
            ...
            console.log('Fetching local specialties for SAGAR...');
        ...
        lastUpdated: new Date().toISOString()
              ...
              const { data: caseInsensitiveResults } = await supabase
                .from('local_specialties')
                .select('id, name, description, city, state, image_url, category, created_at, updated_at, city_id')
                .or('city.ilike.%sagar%,state.ilike.%madhya pradesh%')
                .order('name', { ascending: true });
              ...
              if (caseInsensitiveResults && caseInsensitiveResults.length > 0) {
                ...
                setError('No local specialties found for sagar.');
                setLocalSpecialties([]);
              }
            }
          } catch (err) {
            ...
            if (!response.data || response.data.length === 0) {
              ...
              if (response.data) {
                response.data = response.data.filter((item: TransportOption) => 
                  item.city?.toLowerCase().includes('sagar') ||
                  item.name?.toLowerCase().includes('sagar') ||
                  item.description?.toLowerCase().includes('sagar')
                );
              }
            }
          } else {
            ...
            if (!response.data || response.data.length === 0) {
              ...
              if (response.data) {
                response.data = response.data.filter((item: Event) => 
                  item.city?.toLowerCase().includes('sagar') ||
                  item.location?.toLowerCase().includes('sagar') ||
                  item.name?.toLowerCase().includes('sagar')
                );
              }
            }
    ...
    if (currentData.length === 0) {
      return (
        <div className="text-center py-12">
          <p className="text-gray-500">No {selectedTab.toLowerCase()} found for sagar.</p>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
     ...
     <div className="relative h-96 overflow-hidden">
        <img
          src="https://d3sftlgbtusmnv.cloudfront.net/blog/wp-content/uploads/2024/10/Best-Places-to-Visit-in-Sagar-for-a-Cultural-Experience-840x425.jpg"
          alt="Sagar"
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center text-white max-w-4xl px-4">
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-8 border border-white/20">
              ...
              SWITZERLAND OF MADHYA PRADESH
              </p>
              <p className="text-sm md:text-base mt-2 opacity-75 font-medium">
              CELEBRATED FOR NATURAL BEAUTY & CULTURAL HERITAGE
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="bg-white rounded-xl shadow-sm p-6">
              <h2 className="text-2xl font-bold mb-4">About SAGAR</h2>
              <p className="text-gray-600 mb-6">
              SAGAR – KNOWN AS THE "SWITZERLAND OF MADHYA PRADESH", CELEBRATED FOR NATURAL BEAUTY AND CULTURAL HERITAGE.
              </p>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              <div className="bg-gray-100 rounded-lg h-48 relative overflow-hidden mb-4">
                <iframe
                  src="https://www.google.com/maps?q=Sagar,+Madhya+Pradesh,+India&output=embed"
                  width="100%"
                  height="100%"
                  style={{ border: 0 }}
