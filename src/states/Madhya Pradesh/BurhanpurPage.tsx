...
const BurhanpurPage = () => {
  ...
  const [cityId, setCityId] = useState<string>('burhanpur-mp');
      ...
      switch (selectedTab) {
        ...
        case 'Local Specialties':
          try {
            ...
            console.log('Fetching local specialties for BURHANPUR...');
        ...
        lastUpdated: new Date().toISOString()
              ...
              const { data: caseInsensitiveResults } = await supabase
                .from('local_specialties')
                .select('id, name, description, city, state, image_url, category, created_at, updated_at, city_id')
                .or('city.ilike.%burhanpur%,state.ilike.%madhya pradesh%')
                .order('name', { ascending: true });
              ...
              if (caseInsensitiveResults && caseInsensitiveResults.length > 0) {
                ...
                setError('No local specialties found for burhanpur.');
                setLocalSpecialties([]);
              }
            }
          } catch (err) {
            ...
            if (!response.data || response.data.length === 0) {
              ...
              if (response.data) {
                response.data = response.data.filter((item: TransportOption) => 
                  item.city?.toLowerCase().includes('burhanpur') ||
                  item.name?.toLowerCase().includes('burhanpur') ||
                  item.description?.toLowerCase().includes('burhanpur')
                );
              }
            }
          } else {
            ...
            if (!response.data || response.data.length === 0) {
              ...
              if (response.data) {
                response.data = response.data.filter((item: Event) => 
                  item.city?.toLowerCase().includes('burhanpur') ||
                  item.location?.toLowerCase().includes('burhanpur') ||
                  item.name?.toLowerCase().includes('burhanpur')
                );
              }
            }
    ...
    if (currentData.length === 0) {
      return (
        <div className="text-center py-12">
          <p className="text-gray-500">No {selectedTab.toLowerCase()} found for Burhanpur.</p>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
     ...
     <div className="relative h-96 overflow-hidden">
        <img
          src="https://inditales.com/wp-content/uploads/2018/10/black-taj-mahal-burhanpur.jpg"
          alt="burhanpur"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="bg-white rounded-xl shadow-sm p-6">
              <h2 className="text-2xl font-bold mb-4">About BURHANPUR</h2>
              <p className="text-gray-600 mb-6">
              Burhanpur – The Mughal Gateway to the Deccan, a city of forts, palaces, and timeless heritage.
              </p>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              <div className="bg-gray-100 rounded-lg h-48 relative overflow-hidden mb-4">
                <iframe
                  src="https://www.google.com/maps?q=Burhanpur,+Madhya+Pradesh&output=embed"
                  width="100%"
                  height="100%"
                  style={{ border: 0 }}
