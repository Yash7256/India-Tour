...
const UjjainPage = () => {
  ...
  const [cityId, setCityId] = useState<string>('ujjain-mp');
            ...
            if (!response.data || response.data.length === 0) {
              ...
              if (response.data) {
                response.data = response.data.filter((item: TransportOption) => 
                  item.city?.toLowerCase().includes('ujjain') ||
                  item.name?.toLowerCase().includes('ujjain') ||
                  item.description?.toLowerCase().includes('ujjain')
                );
              }
            }
          } else {
            ...
            if (!response.data || response.data.length === 0) {
              ...
              if (response.data) {
                response.data = response.data.filter((item: Event) => 
                  item.city?.toLowerCase().includes('ujjain') ||
                  item.location?.toLowerCase().includes('ujjain') ||
                  item.name?.toLowerCase().includes('ujjain')
                );
              }
            }
    ...
    if (currentData.length === 0) {
      return (
        <div className="text-center py-12">
          <p className="text-gray-500">No {selectedTab.toLowerCase()} found for Ujjain.</p>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
     ...
     <div className="relative h-96 overflow-hidden">
        <img
          src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSuY6w9s2noOxel2V67L6t_KUQLYaNBF7AntQ&s"
          alt="Ujjain"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="bg-white rounded-xl shadow-sm p-6">
              <h2 className="text-2xl font-bold mb-4">About UJJAIN</h2>
              <p className="text-gray-600 mb-6">
              Ujjain, one of the seven sacred cities of India, is revered as the home of Mahakaleshwar Jyotirlinga and a major center of spirituality, culture, and heritage. It is also famous for hosting the Kumbh Mela on the banks of the Shipra River
              </p>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              <div className="bg-gray-100 rounded-lg h-48 relative overflow-hidden mb-4">
                <iframe
                  src="https://www.google.com/maps?q=Ujjain,+Madhya+Pradesh,+India&output=embed"
                  width="100%"
                  height="100%"
                  style={{ border: 0 }}
