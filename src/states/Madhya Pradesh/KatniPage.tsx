...
const KatniPage = () => {
  ...
  const [cityId, setCityId] = useState<string>('katni-mp');
            ...
            if (!response.data || response.data.length === 0) {
              ...
              if (response.data) {
                response.data = response.data.filter((item: TransportOption) => 
                  item.city?.toLowerCase().includes('katni') ||
                  item.name?.toLowerCase().includes('katni') ||
                  item.description?.toLowerCase().includes('katni')
                );
              }
            }
          } else {
            ...
            if (!response.data || response.data.length === 0) {
              ...
              if (response.data) {
                response.data = response.data.filter((item: Event) => 
                  item.city?.toLowerCase().includes('katni') ||
                  item.location?.toLowerCase().includes('katni') ||
                  item.name?.toLowerCase().includes('katni')
                );
              }
            }
    ...
    if (currentData.length === 0) {
      return (
        <div className="text-center py-12">
          <p className="text-gray-500">No {selectedTab.toLowerCase()} found for katni.</p>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
     ...
     <div className="relative h-96 overflow-hidden">
        <img
          src="https://dynamic-media-cdn.tripadvisor.com/media/photo-o/07/4d/e7/3f/vijayraghavgarh-fort.jpg?w=600&h=400&s=1"
          alt="Katni"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="bg-white rounded-xl shadow-sm p-6">
              <h2 className="text-2xl font-bold mb-4">About KATNI</h2>
              <p className="text-gray-600 mb-6">
              KATNI – LIMESTONE CITY OF INDIA, KNOWN FOR ITS MINERAL RESOURCES AND NATURAL BEAUTY.
              </p>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              <div className="bg-gray-100 rounded-lg h-48 relative overflow-hidden mb-4">
                <iframe
                  src="https://www.google.com/maps?q=Katni,+Madhya+Pradesh,+India&output=embed"
                  width="100%"
                  height="100%"
                  style={{ border: 0 }}
