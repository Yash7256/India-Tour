...
const UmariaPage = () => {
  ...
  const [cityId, setCityId] = useState<string>('umaria-mp');
      ...
      switch (selectedTab) {
        ...
        case 'Local Specialties':
          try {
            ...
            console.log('Fetching local specialties for UMARIA ...');
        ...
        lastUpdated: new Date().toISOString()
              ...
              const { data: caseInsensitiveResults } = await supabase
                .from('local_specialties')
                .select('id, name, description, city, state, image_url, category, created_at, updated_at, city_id')
                .or('city.ilike.%umaria%,state.ilike.%madhya pradesh%')
                .order('name', { ascending: true });
              ...
              if (caseInsensitiveResults && caseInsensitiveResults.length > 0) {
                ...
                setError('No local specialties found for umaria.');
                setLocalSpecialties([]);
              }
            }
          } catch (err) {
            ...
            if (!response.data || response.data.length === 0) {
              ...
              if (response.data) {
                response.data = response.data.filter((item: TransportOption) => 
                  item.city?.toLowerCase().includes('umaria') ||
                  item.name?.toLowerCase().includes('umaria') ||
                  item.description?.toLowerCase().includes('umaria')
                );
              }
            }
          } else {
            ...
            if (!response.data || response.data.length === 0) {
              ...
              if (response.data) {
                response.data = response.data.filter((item: Event) => 
                  item.city?.toLowerCase().includes('umaria') ||
                  item.location?.toLowerCase().includes('umaria') ||
                  item.name?.toLowerCase().includes('umaria')
                );
              }
            }
    ...
    if (currentData.length === 0) {
      return (
        <div className="text-center py-12">
          <p className="text-gray-500">No {selectedTab.toLowerCase()} found for umaria.</p>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
     ...
     <div className="relative h-96 overflow-hidden">
        <img
          src="https://im.hunt.in/cg/mp/Umaria/City-Guide/khauj.jpg"
          alt="img"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="bg-white rounded-xl shadow-sm p-6">
              <h2 className="text-2xl font-bold mb-4">About UMARIA</h2>
              <p className="text-gray-600 mb-6">
              UMARIA – FAMOUS FOR BANDHAVGARH TIGER RESERVE, A MAJOR WILDLIFE DESTINATION, AND COAL MINES, INCLUDING RELIGIOUS SITES AND BANDHAVGARH FORT.
              </p>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              <div className="bg-gray-100 rounded-lg h-48 relative overflow-hidden mb-4">
                <iframe
                  src="https://www.google.com/maps?q=Umaria,+Madhya+Pradesh,+India&output=embed"
                  width="100%"
                  height="100%"
                  style={{ border: 0 }}
